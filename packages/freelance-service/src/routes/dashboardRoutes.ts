...
router.get("/client/:clientId", async (req: Request, res: Response) => {
  try {
    ...
    const gigStats = await Promise.all(gigs.map(async (gig) => {
    ...
    const stats = {
      ...
      activeGigs: gigs.filter(g => g.status !== "completed" && g.status !== "cancelled").length,
      completedGigs: gigs.filter(g => g.status === "completed").length,
      gigs: gigStats
    };

    res.status(responseMapping.SUCCESS.code).json({ success: true, data: stats });
  } catch (error: any) {
